...
async fn test_send_cell_to_recipient_with_non_existing_coinbase(
    ...
    let spent_cell_hash = spend_cell(&from, &to, coinbase_tx, spend_amount).await?;
    assert!(spent_cell_hash.is_none());
...
async fn send_cell_and_get_result(
    ...
    let output_index =
        cell.outputs().iter().position(|o| o.lock == from.public_key).unwrap_or_else(|| 0);
...
fn assert_cell(
    ...
    assert_eq!(2, spent_cell_outputs.len(), "Cell must have spent and remaining outputs");
    ...
    assert_eq!(1, inputs_as_vec.len(), "Only 1 input for a spent output must be in the cell");

    for input in inputs_as_vec {
        assert_eq!(
            output_index as u8, input.output_index.index,
            "Cell input index must be same as the output position of the original cell"
        );
        assert_eq!(
