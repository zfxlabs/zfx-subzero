...
impl ConflictMap {
    pub fn new() -> Self {
        ConflictMap { inner: HashMap::default(), spend_map: SpendMap::new() }
    }
    ...
    pub fn is_preferred(&self, tx_hash: TxHash) -> Result<bool> {
        match self.inner.get(&tx_hash) {
            Some(cs) => Ok(cs.is_preferred(tx_hash)),
            None => Err(Error::InvalidTransactionHash(tx_hash.clone())),
        }
    }
    ...
    pub fn update_conflict_set(&mut self, tx: TxHash, d1: u8, d2: u8) -> Result<()> {
        match self.inner.entry(tx.clone()) {
            ...
            Entry::Vacant(_) => Err(Error::InvalidTransactionHash(tx.clone())),
        }
    }
}
