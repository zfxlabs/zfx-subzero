...
impl Transaction {
    ...
    pub fn inner(&self) -> Tx {
        match self {
            Transaction::CoinbaseTx(tx) => tx.tx.clone(),
            Transaction::StakeTx(tx) => tx.tx.clone(),
            Transaction::TransferTx(tx) => tx.tx.clone(),
        }
    }
    ...
    pub fn outputs(&self) -> Vec<Output> {
        match self {
            ...
            Transaction::TransferTx(tx) => tx.outputs(),
        }
    }

    pub fn hash(&self) -> [u8; 32] {
        match self {
            Transaction::CoinbaseTx(tx) => tx.hash(),
            Transaction::StakeTx(tx) => tx.hash(),
            Transaction::TransferTx(tx) => tx.hash(),
        }
    }
}
